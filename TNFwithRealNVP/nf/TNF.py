...
class RealNVP(nn.Module):
    ...
    #     change1, noChange1 = z1[:,0].reshape(-1, 1), z1[:,1:3]
    #     t2_transformed = self.t2(noChange1)
    #     s2_transformed = self.s2(noChange1)
    #     changed_new1 = t2_transformed + change1 * torch.exp(s2_transformed)
        # noChange1 = noChange1
    #     z2 = torch.cat([changed_new1, noChange1], dim=1)
        
    #     change2, noChange2 = z2[:,0::2], z2[:,1].reshape(-1, 1)
    #     t3_transformed = self.t3(change2)
    #     s3_transformed = self.s3(change2)
    #     changed_new2 = change2
    #     upper_new2 = t3_transformed + noChange2 * torch.exp(s3_transformed)
    #     z3 = torch.cat([changed_new2[:,0].reshape(-1, 1), upper_new2, changed_new2[:,1].reshape(-1, 1)], dim=1)
    ...
    #     change2, noChange2 = x3[:,0::2], x3[:,1].reshape(-1, 1)
    #     t3_transformed = self.t3(change2)
    #     s3_transformed = self.s3(change2)
    #     changed_new2 = change2
    #     upper_new2 = (noChange2 - t3_transformed) * torch.exp(-s3_transformed)        
    #     x2 =  torch.cat([changed_new2[:,0].reshape(-1, 1), upper_new2, changed_new2[:,1].reshape(-1, 1)], dim=1)
        
    #     change1, noChange1 = x2[:,0].reshape(-1, 1), x2[:,1:3]
    #     t2_transformed = self.t2(noChange1)
    #     s2_transformed = self.s2(noChange1)
    #     changed_new1 = (change1 - t2_transformed) * torch.exp(-s2_transformed)
        # noChange1 = noChange1 
    #     x1 = torch.cat([changed_new1[:,0].reshape(-1, 1), noChange1], dim=1)
